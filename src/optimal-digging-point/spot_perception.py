...
class StereoDisparityPublisher(Node):
    ...
    def callback(self, left_msg, right_msg):
        ...
        # YOLOv5 object detection on the left image
        ...
        # Map detected objects to world coordinates
        for *xyxy, conf, cls in detections:
            x1, y1, x2, y2 = map(int, xyxy)
            cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
            disp_value = disparity[cy, cx]
            if disp_value > 0:
                depth = (fx * baseline) / disp_value
                world_x = (cx - cx) * depth / fx
                world_y = (cy - cy) * depth / fy
                world_z = depth
